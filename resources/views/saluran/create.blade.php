@extends('layouts.app')
@section('title', 'Tambah Saluran — Smart Irrigation')
...
<div style="max-width:720px;">
    <div style="margin-bottom:1.5rem;">
        <h2 style="font-family:'Fraunces',serif;font-size:1.5rem;font-weight:700;color:var(--soil);">Tambah Saluran</h2>
        <p style="font-size:.85rem;color:var(--textlt);margin-top:.25rem;">Isi data saluran irigasi baru.</p>
    </div>

    <div class="card">
        <form method="POST" action="{{ route('saluran.store') }}">
            @csrf

            <div class="form-group">
                <label class="form-label">Nama Saluran <span style="color:#a03828;">*</span></label>
                <input type="text" name="nama" class="form-control" value="{{ old('nama') }}" placeholder="Saluran Primer Barito">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                <div class="form-group">
                    ...
                    <select name="tipe" class="form-select">
                        <option value="primer"   {{ old('tipe') === 'primer'   ? 'selected' : '' }}>Primer</option>
                        <option value="sekunder" {{ old('tipe','sekunder') === 'sekunder' ? 'selected' : '' }}>Sekunder</option>
                        <option value="tersier"  {{ old('tipe') === 'tersier'  ? 'selected' : '' }}>Tersier</option>
                    </select>
                    @error('tipe')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Panjang (km)</label>
                    <input type="number" name="panjang_km" class="form-control" value="{{ old('panjang_km') }}" placeholder="0.000" step="0.001" min="0">
