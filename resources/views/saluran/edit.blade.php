@extends('layouts.app')
@section('title', 'Edit Saluran — Smart Irrigation')
...
<div style="margin-bottom:1.5rem;">
    <a href="{{ route('saluran.index') }}" style="font-size:.82rem;color:var(--textlt);text-decoration:none;">← Kembali ke Saluran</a>
</div>

<div style="max-width:720px;">
    <div style="margin-bottom:1.5rem;">
        <h2 style="font-family:'Fraunces',serif;font-size:1.5rem;font-weight:700;color:var(--soil);">Edit Saluran</h2>
        <p style="font-size:.85rem;color:var(--textlt);margin-top:.25rem;">Update data saluran <strong>{{ $saluran->nama }}</strong>.</p>
    </div>

    <div class="card">
        <form method="POST" action="{{ route('saluran.update', $saluran) }}">
            @csrf @method('PUT')

            <div class="form-group">
                <label class="form-label">Nama Saluran <span style="color:#a03828;">*</span></label>
                <input type="text" name="nama" class="form-control" value="{{ old('nama', $saluran->nama) }}">
                @error('nama')<p class="form-error">{{ $message }}</p>@enderror
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                <div class="form-group">
                    <label class="form-label">Tipe <span style="color:#a03828;">*</span></label>
                    <select name="tipe" class="form-select">
                        <option value="primer"   {{ old('tipe', $saluran->tipe) === 'primer'   ? 'selected' : '' }}>Primer</option>
                        <option value="sekunder" {{ old('tipe', $saluran->tipe) === 'sekunder' ? 'selected' : '' }}>Sekunder</option>
                        <option value="tersier"  {{ old('tipe', $saluran->tipe) === 'tersier'  ? 'selected' : '' }}>Tersier</option>
                    </select>
                    @error('tipe')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Panjang (km)</label>
                    <input type="number" name="panjang_km" class="form-control" value="{{ old('panjang_km', $saluran->panjang_km) }}" step="0.001" min="0">
                    @error('panjang_km')<p class="form-error">{{ $message }}</p>@enderror
                </div>
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                <div class="form-group">
                    <label class="form-label">Kondisi <span style="color:#a03828;">*</span></label>
                    <select name="kondisi" class="form-select">
                        <option value="baik"   {{ old('kondisi', $saluran->kondisi) === 'baik'   ? 'selected' : '' }}>Baik</option>
                        <option value="sedang" {{ old('kondisi', $saluran->kondisi) === 'sedang' ? 'selected' : '' }}>Sedang</option>
                        <option value="rusak"  {{ old('kondisi', $saluran->kondisi) === 'rusak'  ? 'selected' : '' }}>Rusak</option>
                    </select>
                    @error('kondisi')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Penanggung Jawab</label>
                    <input type="text" name="penanggung_jawab" class="form-control" value="{{ old('penanggung_jawab', $saluran->penanggung_jawab) }}">
            ...
            <div style="background:var(--cream2);border:1px solid var(--border);border-radius:10px;padding:1.25rem;margin-bottom:1.25rem;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div class="form-group" style="margin-bottom:0;">
                        ...
                        <input type="number" name="latitude" id="latitude" class="form-control" value="{{ old('latitude', $saluran->latitude) }}" step="any">
                        @error('latitude')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                    <div class="form-group" style="margin-bottom:0;">
                        <label class="form-label">Longitude</label>
                        <input type="number" name="longitude" id="longitude" class="form-control" value="{{ old('longitude', $saluran->longitude) }}" step="any">
                        @error('longitude')<p class="form-error">{{ $message }}</p>@enderror
                    </div>
                </div>
                <p class="form-hint" style="margin-top:.75rem;">Klik peta untuk mengubah koordinat.</p>
                <div id="map-picker" style="height:220px;border-radius:8px;margin-top:.75rem;border:1px solid var(--border);"></div>
            </div>

            <div class="form-group">
                <label class="form-label">Keterangan</label>
                <textarea name="keterangan" class="form-control" rows="3">{{ old('keterangan', $saluran->keterangan) }}</textarea>
...
<script>
    const lat = {{ $saluran->latitude ?? -2.5489 }};
    const lng = {{ $saluran->longitude ?? 115.7624 }};
    ...
    if ({{ $saluran->latitude ? 'true' : 'false' }}) {
