@extends('layouts.app')
@section('title', 'RTT {{ $daerahIrigasi->nama }} — Smart Irrigation')
...
<style>
    ...
    .badge-terlambat{ background:rgba(185,74,60,.08);border:1px solid rgba(185,74,60,.2);  color:#a03828; }
...
<div style="margin-bottom:1rem;">
    <a href="{{ route('rtt.index', ['musim_tanam_id' => $mtId]) }}"
        style="font-size:.82rem;color:var(--textlt);text-decoration:none;"
        onmouseover="this.style.color='var(--water)'" onmouseout="this.style.color='var(--textlt)'">
        ← Kembali ke Daftar DI
    </a>
</div>
...
<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
    <div>
        <h2 style="font-family:'Fraunces',serif;font-size:1.6rem;font-weight:700;color:var(--soil);letter-spacing:-.02em;">{{ $daerahIrigasi->nama }}</h2>
        <p style="font-size:.82rem;color:var(--textlt);margin-top:.2rem;">
            RTT per Petak · {{ $daerahIrigasi->kode }}
            @if($mt)
            <span style="margin:0 .4rem;">·</span>
            <strong style="color:var(--soil);">{{ $mt->nama_mt }}</strong>
            <span style="margin:0 .4rem;">·</span>
    ...
    <div style="display:flex;gap:.75rem;align-items:center;">
        <form method="GET" action="{{ route('rtt.by-di', $daerahIrigasi) }}">
    ...
    <div class="card" style="padding:1.5rem;">
        ...
        @if($rtts->isEmpty())
            <div style="text-align:center;padding:3rem;">
                ...
                <p style="font-family:'Fraunces',serif;font-size:1.1rem;font-weight:600;color:var(--soil);">Belum ada RTT untuk DI ini</p>
                ...
                <div id="gantt-container" style="min-width:700px;">
                    ...
                    <div style="margin-top:1rem;padding-top:1rem;border-top:1px solid var(--border);display:flex;flex-wrap:wrap;gap:.5rem;margin-left:160px;">
                        ...
                        <span style="font-size:.72rem;color:var(--textlt);display:flex;align-items:center;gap:.3rem;">
                            <span style="width:10px;height:10px;border-radius:2px;background:{{ $val[0] }};display:inline-block;"></span>{{ $val[1] }}
                        </span>
                        @endforeach
                    </div>
                </div>
    ...
    function renderGantt() {
        ...
        if (header) {
            ...
            while (cur <= lastMonth) {
                ...
                bulanHTML += `<div style="flex:0 0 ${pct}%;min-width:0;font-size:.65rem;font-weight:700;color:var(--textlt);text-align:center;border-left:1px solid var(--border);padding:.25rem 0;overflow:hidden;">${bulanNama[m]} ${y}</div>`;
                [{ label:'I',start:1,end:10 },{ label:'II',start:11,end:20 },{ label:'III',start:21,end:daysInMonth }].forEach(dek => {
                    const ds = new Date(y, m, dek.start), de = new Date(y, m, dek.end);
                    if (de < minDate || ds > maxDate) return;
                    const effDS = ds < minDate ? minDate : ds, effDE = de > maxDate ? maxDate : de;
                    const dDays = Math.round((effDE - effDS) / 86400000) + 1;
                    const dPct  = (dDays / totalDays) * 100;
                    dekadeHTML += `<div style="flex:0 0 ${dPct}%;min-width:0;font-size:.58rem;font-weight:600;color:var(--textlt);text-align:center;border-left:1px solid rgba(139,94,60,.08);padding:.15rem 0;overflow:hidden;">${dek.label}<br><span style="font-size:.52rem;opacity:.7;">${dek.start}–${dek.end}</span></div>`;
                });
                cur = new Date(y, m+1, 1);
            }
            header.style.flexDirection = 'column';
            header.innerHTML = `<div style="display:flex;width:100%;border-bottom:1px solid rgba(139,94,60,.1);">${bulanHTML}</div><div style="display:flex;width:100%;border-bottom:2px solid var(--border);">${dekadeHTML}</div>`;
        }

        ganttData.forEach(rtt => {
            const barEl  = document.getElementById(`gantt-bar-${rtt.id}`);
            const faseEl = document.getElementById(`gantt-fase-${rtt.id}`);
            if (!barEl) return;
            const mulai = new Date(rtt.mulai), selesai = new Date(rtt.selesai);
            const leftPct  = ((mulai - minDate) / 86400000 / totalDays) * 100;
            const widthPct = ((selesai - mulai) / 86400000 / totalDays) * 100;
            let html = `<div class="gantt-bar gantt-bar-plan" style="left:${leftPct}%;width:${widthPct}%;">${widthPct > 8 ? rtt.petak : ''}</div>`;
            if (rtt.mulai_real) {
                const mulaiR = new Date(rtt.mulai_real), selesaiR = rtt.selesai_real ? new Date(rtt.selesai_real) : new Date();
            ...
            if (faseEl && rtt.jadwal_fase.length) {
                ...
                rtt.jadwal_fase.forEach(f => {
                    const fm = new Date(f.mulai), fs = new Date(f.selesai);
